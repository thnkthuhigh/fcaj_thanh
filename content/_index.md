---
title: "Internship Report"
date: 2024-01-01
weight: 1
chapter: false
---

<h1 style="text-align: center; color: #5e728b;">INTERNSHIP REPORT</h1>

<div style="margin: 1.5rem 0; padding: 1.25rem 1.5rem; border: 2px solid #ff9900; border-radius: 14px; background: #fff8ec; box-shadow: 0 8px 24px rgba(255, 153, 0, 0.14);">
  <h3 style="margin-top: 0; color: #232f3e;">Grading materials and video demo</h3>
  <p style="margin-bottom: 1rem;">Please open the Google Drive folder below to review the project demo video, AWS screenshots, and supporting evidence for grading.</p>
  <a href="https://drive.google.com/drive/folders/1-mvKrbzs08WWHUT0-whSGeNwgtwd4Uid" target="_blank" rel="noopener noreferrer" style="display: inline-block; padding: 0.75rem 1rem; border-radius: 999px; background: #ff9900; color: #111827; font-weight: 700; text-decoration: none;">
    Open video demo and grading evidence
  </a>
</div>

### Student Information:
<div class="student-profile">
  <div class="student-profile__info">
    <p><strong>Full Name:</strong> Nguyen Chi Thanh</p>
    <p><strong>Phone Number:</strong> 0359498968</p>
    <p><strong>Email:</strong> ngchithanh.dev@gmail.com</p>
    <p><strong>University:</strong> HUTECH University of Technology</p>
    <p><strong>Major:</strong> Software Engineering</p>
    <p><strong>Student ID:</strong> 2280602915</p>
    <p><strong>Class:</strong> 22DTHD7</p>
    <p><strong>Internship Company:</strong> Amazon Web Services Viet Nam Company Limited</p>
    <p><strong>Internship Position:</strong> Workforce Bootcamp - First Cloud AI Journey</p>
    <p><strong>Internship Duration:</strong> 17/04/2026 - 10/07/2026</p>
  </div>
  <div class="student-profile__avatar">
    <img src="/fcaj_thanh/images/avatar-thanh.jpg" alt="Profile picture of Nguyen Chi Thanh" style="width: 100%; max-width: 320px; height: auto; border-radius: 18px; box-shadow: 0 10px 30px rgba(15, 40, 75, 0.12); object-fit: cover;">
  </div>
</div>

### Report Content

1. [Worklog](1-Worklog/)
2. [Proposal](2-Proposal/)
3. [Translated Blogs](3-BlogsTranslated/)
4. [Events Participated](4-EventParticipated/)
5. [Workshop](5-Workshop/)
6. [Self-assessment](6-Self-evaluation/)
7. [Sharing and Feedback](7-Feedback/)
